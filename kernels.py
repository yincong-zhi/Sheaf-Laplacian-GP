...
class SheafGGP(gpflow.kernels.Kernel):
    ...
    def K(self, X, X2=None):
        X = tf.cast(tf.reshape(X, [-1]), dtype=tf.int32)
        X2 = tf.cast(tf.reshape(X2, [-1]), dtype=tf.int32) if X2 is not None else X
        inner_cov = self.base_kernel.K(self.node_feats)
        S = self.variance * tf.linalg.inv(tf.eye(num_nodes, dtype = tf.float64) + self.alpha * self.sheaf_construct(self.sheaf_learner))
